...
int ls(int argc, char **argv)
{
    DIR dir;
    FILINFO file;
    int ret = opendir(&dir, argv[1]);
    if(ret <0)
    {
      cprintf("File or path not exist.\n");
      return 0;
    }
    ret = readdir(&dir, &file);
    while(!ret && file.fname[0] != 0)   /* Break on error or end of dir */
    {
      cprintf("%s ", file.fname );
...
int touch(int argc, char **argv)
{
  ...
  char current[80];
  strcpy(current, dir[0]);
  for(i=0;i<j;i++)
  {
    if(i!=0)
    {
      strcat(current, "/");
      strcat(current, dir[i]);
    }
    DIR dir_obj;
    int ret = opendir(&dir_obj, current);
    if(ret <0)
    {
      mkdir(current);
    }
