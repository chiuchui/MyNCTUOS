...
void ide_initialize(unsigned int BAR0, unsigned int BAR1, unsigned int BAR2, unsigned int BAR3, unsigned int BAR4)
{
	...
	// We will select the master drive of each channel, 
	// and send the ATA_IDENTIFY command (which is supported by ATA Drives). 
	// If there's no error, there are values returned in registers which determine the type of Drive; 
	// if no drive is present, there will be strange values.
	// Notice that if bit 4 in HDDEVSEL is set to 1, we are selecting the slave drive, 
	// if set to 0, we are selecting the master drive.
	for (i = 0; i < 2; i++)
		for (j = 0; j < 2; j++) {
